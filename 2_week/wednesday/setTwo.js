...
function contains(arr, someName, cb) {
  if (arr.includes(someName)) {
    cb(true);
  } else {
    cb(false);
  }
...
// CODE HERE
const each = (arr, cb) => arr.forEach((val, index) => cb(val, index));
...
// CODE HERE
each(names, (val, index) => `The item at index ${index} is ${val}. `);
